...
function torRequest(options, done) {
  ...
  tr.TorControlPort.password = "wM0XZoaJ6urBtW";
  tr.TorControlPort.port = TOR_CONTROL_PORT;
  tr.setTorAddress('localhost', TOR_PORT);
...
function onrequest(req, res) {
  ...
  authenticate(server, req, function(err, auth) {
    ...
    torRequest(parsed, function(proxyReq) {
